    ...
    snapshot.forEach(docSnap => {
      ...
      if (data.isActive === true) {
        ...
        if (data.deviceId === deviceId) {
          foundDevice = { id: docSnap.id, ...data }
...
// ============================================
// 11. الحصول على طلبات الأجهزة المعلقة - معدل (مع التصحيح)
// ============================================
export const getPendingDeviceRequests = async (): Promise<{
  ...
  try {
    ...
    const requests: any[] = []
    // ✅ غيرنا اسم المتغير من doc إلى deviceDoc
    for (const deviceDoc of snapshot.docs) {
      ...
      console.log(`🔍 معالجة طلب ID: ${deviceDoc.id}`, data)
      ...
      try {
        if (data.userId) {
          ...
          if (userDoc.exists()) {
            ...
            console.log(`✅ تم جلب بيانات المستخدم: ${studentName}`)
          } else {
            console.log(`❌ المستخدم ${data.userId} غير موجود في قاعدة البيانات`)
            console.log(`🔍 نحاول البحث برقم الهاتف...`)
            const usersRef = collection(db, 'users')
            const q2 = query(usersRef, where('phone', '==', data.userId))
            ...
            if (!userSnapshot.empty) {
              ...
              studentPhone = userData.phone || 'غير معروف'
              studentGrade = userData.grade || 'غير محدد'
              console.log(`✅ تم جلب بيانات المستخدم برقم الهاتف: ${studentName}`)
            }
          }
        } else {
          console.log('❌ الطلب بدون userId:', deviceDoc.id)
        }
      } catch (e) {
    ...
    console.log(`📊 الطلبات بعد المعالجة:`, requests)
