...
export default function PlatformPage() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        // ✅ التحقق من وضع المعلم أولاً
        const impersonateData = localStorage.getItem('impersonating');
        let targetUserId = null;
        let isImpersonating = false;
        
        if (impersonateData) {
          try {
            const data = JSON.parse(impersonateData);
            if (data.isImpersonating) {
              isImpersonating = true;
              targetUserId = data.studentId;
              setImpersonatedStudentId(data.studentId);
        ...
        if (isImpersonating && targetUserId) {
          ...
          if (userDoc.exists()) {
            ...
            if (studentData.grade && targetUserId) {
              await fetchCourses(studentData.grade, targetUserId);
            }
            setLoading(false);
            return;
          }
        ...
        // لو مش في وضع معلم، جيب بيانات المستخدم العادي
  ...
  const fetchCourses = async (userYear: string, studentId: string) => {
    try {
      ...
      setFetchError(error?.message || 'فشل تحميل الكورسات')
      setCourses([])
    } finally {
            ...
            if (data.isImpersonating) {
              return (
                <div style={{
                  ...
                  <div>
                    👁️ <strong>وضع المعلم:</strong> أنت تشاهد حساب <strong>{data.studentName}</strong>
                  </div>
                  <button
                    onClick={() => {
