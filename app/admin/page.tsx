  ...
  return (
    <div style={styles.tabContent}>
      <div style={styles.tabHeader}>
        <h2 style={styles.tabTitle}>📱 إدارة أجهزة الطلاب</h2>
        <button 
      ...
      {showRequests ? (
        ...
        <>
          ...
          {loading ? (
            ...
            <p style={styles.emptyText}>لا توجد طلاب في هذه المرحلة</p>
          ) : (
            <div style={styles.devicesStudentsGrid}>
              {filteredStudents.map(student => (
                <div key={student.id} style={styles.deviceStudentCard}>
                  <div style={styles.deviceStudentInfo}>
                    <div style={styles.deviceStudentAvatar}>
                      {student.name.charAt(0)}
                    </div>
                    <div>
                      <h4 style={styles.deviceStudentName}>{student.name}</h4>
                      <p style={styles.deviceStudentDetail}>📱 {student.phone}</p>
                      <p style={styles.deviceStudentDetail}>📚 {getGradeName(student.grade)}</p>
                    </div>
                  </div>
                  <Link
                    href={`/admin/device-management?studentId=${student.id}`}
                    style={styles.deviceManageButton}
                  >
                    📱 إدارة الأجهزة
                  </Link>
                </div>
...
function UpgradeTab() {
  ...
  const gradeOrder = [
    ...
    { code: '3-secondary', name: 'ثالثة ثانوي' }
  ]
...
function TeacherLogs() {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        const q = query(collection(db, "teacher_logs"), orderBy("timestamp", "desc"))
  ...
  if (loading) return <div style={styles.loadingText}>جاري التحميل...</div>

  return (
    <div style={styles.tabContent}>
      <h2 style={styles.tabTitle}>👨‍🏫 سجل دخول المعلمين</h2>
          ...
          <tbody>
            {logs.map((log, idx) => (
              <tr key={log.id} style={styles.tr}>
                ...
                <td style={styles.td}>{log.teacherName}</td>
                <td style={styles.td}>{log.studentName}</td>
                <td style={styles.td}>{log.action === 'view' ? '👁️ مشاهدة' : log.action}</td>
                <td style={styles.td}>{new Date(log.timestamp?.seconds * 1000).toLocaleString('ar-EG')}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function CoursesTab() {
  ...
  const secondaryCategories = ['كيمياء', 'فيزياء']
  ...
  const handleAddCourse = async (e: React.FormEvent) => {
    ...
    if ((newCourse.grade === '2-secondary' || newCourse.grade === '3-secondary') && !newCourse.category) {
      setMessage('❌ يجب اختيار الفولدر (كيمياء أو فيزياء) للمرحلة الثانوية')
      return
    }
  ...
  const handleUpdateCourse = async () => {
    ...
    try {
      ...
      if ((newCourse.grade === '2-secondary' || newCourse.grade === '3-secondary')) {
  ...
  const getCoursesByGradeAndCategory = () => {
    ...
    if (activeGrade !== '2-secondary' && activeGrade !== '3-secondary') {
    ...
    filtered.forEach(course => {
      if (course.category && secondaryCategories.includes(course.category)) {
  ...
  const isSecondary = activeGrade === '2-secondary' || activeGrade === '3-secondary'
  ...
  return (
    <div style={styles.tabContent}>
      ...
      <div style={styles.viewTabs}>
        ...
        >
          🎓 ثالثة ثانوي ({gradeStats['3-secondary']})
        </button>
      </div>
      ...
      <div style={styles.formSection}>
        ...
        <form onSubmit={editingCourse ? handleUpdateCourse : handleAddCourse} style={styles.form}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setNewCourse({
                  ...newCourse, 
                  grade: selectedGrade,
                  category: (selectedGrade === '2-secondary' || selectedGrade === '3-secondary') ? newCourse.category : ''
                })
              }}
              style={styles.input}
            >
          ...
          {(newCourse.grade === '2-secondary' || newCourse.grade === '3-secondary') && (
            <div style={styles.formRow}>
              <select
                ...
                required
              >
                <option value="">اختر الفولدر للمرحلة الثانوية *</option>
      ...
      <div style={styles.listSection}>
            ...
            {Object.keys(categorizedCourses).map(category => (
              <div key={category} style={styles.categorySection}>
                <h4 style={styles.categoryTitle}>
                  {isSecondary ? (
                    category === 'أخرى' ? '📁 كورسات أخرى' : `📁 ${category}`
                  ) : (
                      ...
                      <div style={styles.courseHeader}>
                        <div>
                          ...
                          <div style={styles.courseBadges}>
                            ...
                            {course.category && (course.grade === '2-secondary' || course.grade === '3-secondary') && (
                              <span style={{
        ...
        <div style={styles.gradeStatsSection}>
          ...
          <div style={styles.statsGrid}>
            ...
            <div style={styles.statCard}>
              <div style={styles.statNumber}>{gradeStats['3-secondary']}</div>
              <div style={styles.statLabel}>ثالثة ثانوي</div>
            </div>
            <div style={styles.statCard}>
